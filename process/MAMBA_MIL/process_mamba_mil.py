...
from utils.loop_utils import train_loop, val_loop, train_loop_class_imbalance
...
def process_MAMBA_MIL(args):
    ...
    # model_type = args.General.model_type

    # act = get_act(args.Model.act)
    # mil_model = MambaMIL(in_dim=in_dim, n_classes=num_classes,dropout=dropout,act=act) 
    # mil_model = MambaMIL(in_dim=in_dim, n_classes=num_classes,dropout=dropout)  

    if args.General.MODEL_NAME=="MAMBA_MIL":  
        print("General.MODEL_NAME:",args.General.MODEL_NAME)
        mil_model = MambaMIL(in_dim=in_dim, n_classes=num_classes,dropout=dropout,type="Mamba") 
    elif args.General.MODEL_NAME=="BiMAMBA_MIL":  
        print("General.MODEL_NAME:",args.General.MODEL_NAME)
        mil_model = MambaMIL(in_dim=in_dim, n_classes=num_classes,dropout=dropout,type="BiMamba") 
    else: 
        print("General.MODEL_NAME:",args.General.MODEL_NAME)
        mil_model = MambaMIL(in_dim=in_dim, n_classes=num_classes,dropout=dropout,type="SRMamba")   
    ...
    for epoch in tqdm(range(args.General.num_epochs),colour='GREEN'):
        ...
        #train_loss,cost_time = train_loop_class_imbalance(device,mil_model,train_dataloader,optimizer,now_scheduler) 
