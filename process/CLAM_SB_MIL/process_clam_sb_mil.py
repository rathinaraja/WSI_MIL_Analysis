...
def process_CLAM_SB_MIL(args):
    ...
    if args.Model.criterion in ['ce', 'bce']:
        criterion = get_criterion(args.Model.criterion)
    else:
        df = pd.read_csv(args.Dataset.dataset_csv_path)
        counts = df['train_label'].value_counts()
        # print(f"Class 0: {counts[0]}, Class 1: {counts[1]}")
        total = counts[0] + counts[1]
        alpha_1 = counts[1]/total
        alpha_2 = counts[0]/total   
        criterion = get_criterion(args.Model.criterion, [alpha_1, alpha_2])
