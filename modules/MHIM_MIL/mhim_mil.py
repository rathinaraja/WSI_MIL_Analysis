...
class MHIM_MIL(nn.Module):
    ...
    #     # **MEMORY SAFEGUARD: For large WSIs during inference, use simple attention instead of self-attention**
    #     use_simple_attn_fallback = False
    #     if self.baseline == 'selfattn' and ps > 8000 and not self.training:
    #         print(f"  ! Warning: {ps} patches detected. Using simple attention fallback to prevent OOM.")
    #         use_simple_attn_fallback = True
        
    #     # Encode
    #     if self.baseline == 'selfattn' and not use_simple_attn_fallback:
    #         # **For self-attention with large inputs, disable attention weight computation**
    #         if return_WSI_attn and ps > 5000:
    #             print(f"  ! Warning: {ps} patches - disabling attention weights to save memory")
    #             cls_feat = self.online_encoder(
    #                 x, mask_ids=mask_ids, len_keep=len_keep, 
    #                 return_attn=False, mask_enable=(mask_ids is not None)
    #             )
    #             attn_weights = None
    #         elif return_WSI_attn:
    #             cls_feat, attn_weights = self.online_encoder(
    #                 x, mask_ids=mask_ids, len_keep=len_keep, 
    #                 return_attn=True, mask_enable=(mask_ids is not None)
    #             )
    ...
    #             attn_weights = None
    #     else:
    #         # Simple attention baseline (or fallback for large self-attention cases)
    #         if mask_ids is not None and not use_simple_attn_fallback:
    #             # Apply masking
    #             mask_ids = mask_ids.squeeze(0)
    #             x = x[:, mask_ids, :]
            
    #         # For simple attention, use the Attention module
    #         if use_simple_attn_fallback:
    #             # Create temporary simple attention module if needed
    #             if not hasattr(self, '_fallback_attention'):
    #                 from modules.MHIM_MIL.mhim_mil import Attention
    #                 act_str = 'relu' if isinstance(self.patch_to_emb[1], nn.ReLU) else 'gelu'
    #                 self._fallback_attention = Attention(
    #                     input_dim=x.shape[-1], 
    #                     act=act_str, 
    #                     dropout=True
    #                 ).to(x.device)
    #             cls_feat, attn_weights = self._fallback_attention(x, no_norm=not return_WSI_attn)
    #         else:
    #             cls_feat, attn_weights = self.online_encoder(x, no_norm=not return_WSI_attn)
