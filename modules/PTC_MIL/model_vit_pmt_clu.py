...
class ViT_PMT_CLU(nn.Module):
    def __init__(self, n_classes, args, top_k=1, n_layers=2):
        ...
        self._fc1 = nn.Sequential(nn.Linear(in_dim, emb_dim), nn.ReLU())
        ...
        # Don't create identity matrix here - create it dynamically based on device
    ...
    def forward(self, h, result_dict=None):
        ...
        # h shape: [batch=1, N, in_dim] -> squeeze to [N, in_dim]
        h = h.squeeze(0) if h.dim() == 3 else h
        
        h = self._fc1(h)
    
        #---->prompt_token
        if result_dict is None: 
            pmt_tokens = self.pmt_token.to(device)
        else:
            pmt_tokens = result_dict['pmt_tokens'].to(device)
        
        pmt_tokens = pmt_tokens.squeeze(0)
        h = torch.cat((pmt_tokens, h), dim=0)
        
        #---->cls_token
        cls_tokens = self.cls_token.to(device)
        ...
        if self.args.moving_pmt:
            if result_dict is None:
                ...
                pmt_tokens_expanded = pmt_tokens.unsqueeze(0)
                new_pmt_tokens = (1-self.args.momentum) * pmt_tokens_expanded + self.args.momentum * new_pmt_tokens
        else:
            new_pmt_tokens = new_pmt_tokens
        ...
        for i in range(self.cluster_number):
            p = new_pmt_tokens[:, i, :].unsqueeze(1)
        ...
        idx_cluster_new = torch.tensor(idx_cluster_new, device=device)
    
        h = torch.cat((cls_token, h_global), dim=1)
        ...
        if self.args.merge_token:
            ...
            merge_token = h_before_merge.mean(dim=0, keepdim=True)
    
        # Create identity matrix on the correct device
        identity = torch.eye(new_pmt_tokens.size(1), new_pmt_tokens.size(1), device=device)
        PTC_loss = torch.norm(new_pmt_tokens @ torch.transpose(new_pmt_tokens, 1, 2) - identity)
       
        #---->predict
        ##---->mean cls and merge token to involve prediction
        cls_token_output = cls_token_output.unsqueeze(0)
        
        if merge_token.dim() == 3:
            merge_token = merge_token.squeeze(0)
        
        token_output = torch.cat((cls_token_output, merge_token), dim=0)
        token_output = torch.mean(token_output, dim=0)
        logit = self._fc3(token_output)
        
        Y_hat = torch.argmax(logit, dim=0).reshape(1, 1)
        Y_prob = F.softmax(logit, dim=0).unsqueeze(dim=0)
        logits = logit.unsqueeze(0)
    
        # Return as dictionary to match loop_utils.py expectations
        return_dict = {
            'logits': logits,           # [1, n_classes]
            'Y_prob': Y_prob,           # [1, n_classes]
            'Y_hat': Y_hat,             # [1, 1]
            'PTC_loss': PTC_loss,       # scalar
            'pmt_tokens': new_pmt_tokens.detach(),
            'h_before_merge': h_before_merge,
