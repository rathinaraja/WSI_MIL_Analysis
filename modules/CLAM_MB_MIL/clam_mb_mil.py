...
class CLAM_SB_MIL(nn.Module):
    ...
    def inst_eval(self, A, h, classifier): 
        ...
        # Fix 1: Get the number of patches from dim 1
        k = min(self.k_sample, A.size(1))
        if k == 0:
            return None, None, None  # Handle empty bags
        
        # Fix 2: Use 'k' instead of 'self.k_sample', and ensure dim=1 is specified for both
        ...
        # Fix 3: Use 'k' for target creation
    ...
    def inst_eval_out(self, A, h, classifier):
        ...
        # Fix: Apply the same dynamic 'k' calculation here
        k = min(self.k_sample, A.size(1))
        if k == 0:
            return None, None, None

        # Fix: Use 'k' and specify dim=1
        top_p_ids = torch.topk(A, k, dim=1)[1][-1]
        top_p = torch.index_select(h, dim=0, index=top_p_ids)
        
        # Fix: Use 'k' for target creation
