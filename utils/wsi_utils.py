...
class WSI_Dataset(Dataset):
    def __init__(self, dataset_info_csv_path, group, args):
        ...
        # deciding to use the entire file to randomly split train, test, val or        
        if args.Dataset.dataset_root_dir == {} and args.Dataset.dataset_csv_path != None:
            self.slide_path_list = self.dataset_df['slide_path'].dropna().to_list() 
            self.labels_list = self.dataset_df['label'].dropna().to_list()
        else:
...
class WSI_Dataset_Test(Dataset):
    def __init__(self, dataset_info_csv_path, args, data_split, inference_mode=False): 
        self.dataset_info_csv_path = dataset_info_csv_path
        self.dataset_df = pd.read_csv(self.dataset_info_csv_path)
        self.inference_mode = inference_mode
        
        # Always get slide paths
        self.slide_path_list = self.dataset_df[f'{data_split}_slide_path'].dropna().to_list()
        
        # Get labels only if they exist
        if f'{data_split}_label' in self.dataset_df.columns:
            self.labels_list = self.dataset_df[f'{data_split}_label'].dropna().to_list()
        else:
            self.labels_list = []
...
class WSI_Dataset_Inference(Dataset):
    def __init__(self, dataset_info_csv_path, args): 
        self.dataset_info_csv_path = dataset_info_csv_path
        self.dataset_df = pd.read_csv(self.dataset_info_csv_path)   
        self.slide_path_list = self.dataset_df['slide_path'].dropna().to_list()     
