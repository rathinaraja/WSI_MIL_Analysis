...
def Balanced_Train_Val(args):
    ...
    # Pad to same length
    max_len = max(len(X_train), len(X_val))
    ...
    result = pd.DataFrame({
        ...
        'test_slide_path': pd.Series([pd.NA] * max_len),
        'test_label': pd.Series([pd.NA] * max_len)
    })
...
if __name__ == '__main__':
    ...
    assert abs(args.train_ratio + args.val_ratio - 1.0) < 1e-6, \
        'train_ratio + val_ratio must equal 1'
...
# 3. **Empty test columns**: Use `pd.NA` for proper empty columns
...
# Split ratios: Train=0.7, Val=0.3

# Train=700 (70.0%), Val=300 (30.0%), Total=1000
